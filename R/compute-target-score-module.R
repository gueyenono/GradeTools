...
compute_target_score_module <- function(input, output, session, grade_categories, scores, tidy_performance, current_grade){
  ...
  target_score <- eventReactive(input$submit, {
    ...
    (ni + 1) * (((sum_wi * G_star) - partial_sum_weigthed_avg) / wi) - sum_Xik
 
  })
  
  observeEvent(input$submit, {
    
    print(paste("Your target score is:", target_score()))
    print(paste("Your target grade category is:", input$target_grade_category))
